...
exports.getParserArgs = function(node, state, opts) {
	...
	var defaultId = opts.defaultId || undefined,
		...
		name = node.nodeName,
		ns = node.getAttribute('ns') || CONST.IMPLICIT_NAMESPACES[name] || CONST.NAMESPACE_DEFAULT,
		createMethod = node.getAttribute('toCreate'),
		addMethod = node.getAttribute('toAdd') || CONST.ADD_DEFAULT,
		removeMethod = node.getAttribute('toRemove') || CONST.REMOVE_DEFAULT,
		fullname = ns + '.' + name,
