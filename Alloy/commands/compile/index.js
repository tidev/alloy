...
function compile(args, program) {
	...
	*/
	...
	U.copyAlloyDir(alloyRoot, 'lib', compileConfig.dir.resources); //resourcesDir);
...
function parseView(viewName,dir,viewid,manifest) {
	var template = {
		...
		controllerCode: '',
		lifecycle: '',
		CFG: compileConfig.runtimeConfig // generatedCFG
	};
	var state = { parent: {} };
	var vd = dir ? path.join(dir,'views') : compileConfig.dir.views; // viewsDir;
	var sd = dir ? path.join(dir,'styles') : compileConfig.dir.styles; // stylesDir;
...
function generateController(name, dir, id) {
	var controllerDir = dir ? path.join(dir,'controllers') : compileConfig.dir.controllers, //controllersDir;
...
function findAndLoadModels() {
	var f = compileConfig.dir.models; // modelsDir;
	var code = '';
	if (!path.existsSync(f)) {
		wrench.mkdirSyncRecursive(f, 777);
	}		
...
function copyBuiltins() {
	...
	// compile time, only copying the libraries that we 
	// actually require in our app - saving space and memory
	var builtInsDir = compileConfig.dir.builtins; // path.join(alloyRoot,'builtins');
	...
	var alloyLibs = [];
	var resourcesDir = compileConfig.dir.resources; //path.join(outputPath,'Resources');
	var files = wrench.readdirSyncRecursive(resourcesDir);
	_.each(files, function(file) {
		var ext = file.substring(file.length-3);
		if (ext == '.js') {
	...
	if (alloyLibs.length > 0) {
		...
		// the project relative to the alloy directory so that 
		// when they are required in the real project they will be available
		var alloyDir = compileConfig.dir.resourcesAlloy; //path.join(resourcesDir,'alloy');
		alloyLibs = _.uniq(alloyLibs);
		_.each(alloyLibs,function(lib) {
			// find all dependencies that look to be relative to our dependency
			var depends = requires.findAllRequires(lib);
