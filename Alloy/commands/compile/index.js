...
module.exports = function(args, program) {
	...
	// check if DefaultIcon.png is in app directory
	var defaultIcons = ['DefaultIcon.png', 'DefaultIcon-' + buildPlatform + '.png'];
	defaultIcons.forEach(function (file) {
		if (fs.existsSync(path.join(paths.project, file)) && !fs.existsSync(path.join(paths.app, file))) {
			U.die([
				'Detected legacy "DefaultIcon.png/DefaultIcon-<platform>.png" in project directory.',
				'Please move "DefaultIcon.png/DefaultIcon-<platform>.png" to "/app" for Alloy 1.10.0 or later.'
			]);
		}
	});

	var defaultIconSrcDir = paths.app;
	...
	if (theme) {
		...
		defaultIconSrcDir = path.join(paths.app, 'themes', theme);
		if (!fs.existsSync(defaultIconSrcDir)) {
			defaultIconSrcDir = paths.app;
		}
	}

	defaultIcons.forEach(function (file) {
		var src = path.join(defaultIconSrcDir, file);
